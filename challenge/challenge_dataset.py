...
def random_rotation(image: np.ndarray, max_angle: float = 15.0) -> np.ndarray:
    """Randomly rotate an image by up to ±max_angle degrees."""
    angle = random.uniform(-max_angle, max_angle)
    img = Image.fromarray(image)
    return np.array(img.rotate(angle))


def random_brightness(image: np.ndarray, max_change: float = 0.2) -> np.ndarray:
    """Randomly adjust brightness by up to ±max_change."""
...
def random_noise(image: np.ndarray, noise_std: float = 5.0) -> np.ndarray:
...
def apply_augmentations(image: np.ndarray) -> np.ndarray:
    """Randomly apply augmentations in sequence."""
    augmentations = [random_flip, random_rotation, random_brightness, random_noise]
...
class ImageStandardizer:
    ...
    def transform(self, X: npt.NDArray) -> npt.NDArray:
        ...
        # TODO: 1(a) - Complete this function
        X_standard = (X - self.image_mean)/self.image_std
        return X_standard
...
class DogsDataset(Dataset):
    ...
    def __init__(self, partition: str, task: str = "target", augment: bool = False) -> None:
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # If image is stored as (C, H, W), convert to (H, W, C) for PIL augmentations
        if image.ndim == 3 and image.shape[0] in [1, 3]:
            image = image.transpose(1, 2, 0)

        # --- Apply augmentations before standardization ---
        if self.augment and self.partition == "train":
        ...
        # Convert back to (C, H, W) for PyTorch
